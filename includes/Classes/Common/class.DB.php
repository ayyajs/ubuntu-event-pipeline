...
class classDB
{
	...
	*/		
	...
	{
		global $_REQUEST;
		$this->dbConnected 	= $_REQUEST["DB_CONNECTED"] ?? 0;
		$this->dbLink		= $_REQUEST["DB_LINK"] ?? null;		
		if ($this->dbConnected == 0) {
			$this->dbLink = mysqli_connect($this->dbServer, $this->dbUser, $this->dbPass) or $this->reportError($this->dbLink,"Could not connect to Server.");
			//$this->doPrint("CONNECTING DATABASE : LINK => ".$this->dbLink);
			$_REQUEST["DB_CONNECTED"] 	= $this->dbConnected = 1;
			$_REQUEST["DB_LINK"] 		= $this->dbLink;	
			mysqli_select_db ($this->dbLink, $this->dbDatabase) or $this->reportError($this->dbLink,"Could not select Database.");	
		} 
	}
	function dbDisconnect()
	{
		// mysqli_close is not necessary, non-persistent links are automatically closed at script end.
...
//					print $this->dbQuery."<br>";
				switch($this->dbType) {
					case 1:
						$this->dbQueryResult = mysqli_query ($this->dbLink,$this->dbQuery) or $this->reportError($this->dbLink);
						$global_log[] = $this->dbQuery;						
						break;	
					case 2:
						$this->dbQueryResult = mysqli_query ($this->dbLink,$this->dbQuery) or $this->reportError($this->dbLink,"Error in SQL Query.");
						$global_log[] = $this->dbQuery;
						break;
				}					
		...
		if ($this->dbResultSetInMem == 1) {
			switch ($this->dbType) {
				case 1:
					...
					break;	
				case 2:
					mssql_free_result($this->dbQueryResult);
					break;
			}				
		} else {
			// nothing
	...
	function dbError($error_no)
	{
		...
		$this->reportError($this->dbLink,$error_desc);
	}
	function reportError($dbLink,$error = "")
	{ // Sends Error Report 1. To logfile 2. To Email 3. system logs
		print $error;
		global $config;
		$log_file_name = date("M_Y")."_logs.txt";
		$log_time = date("D d-M-Y H:i:s");
		$log_string = "[".$log_time."] ".$error;
		//print mysqli_error()."<br>CHECK THE URL => <br>".$this->dbQuery;
		print mysqli_error($dbLink)."<br>CHECK THE URL => <br>".$this->dbQuery;
		switch ($global_config["error_report"]) {
			case 1:	// reportError in logfile			
				echo $this->dbQuery."<br>".mysqli_error($dbLink);
		...
		if($global_config["debug_mode"]) {
			switch ($this->dbType) {
				case 1:
					$error_string = "<b>Error: </b><i>".$error."</i><br><b>Reason: </b>".mysqli_error($dbLink);		
					break;	
				case 2:
